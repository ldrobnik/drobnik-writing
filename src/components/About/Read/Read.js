  ...
  return (
    <ReadWrapper>
      <AnimatedContent pose={!props.reload ? "visible" : "hidden"}>
        ...
        />
        <ReadMessage>
          {WEBSITE_TEXT_ABOUT.read.introduction[props.lang]}
        </ReadMessage>
      </AnimatedContent>
      <Waypoint onEnter={showLinks} />
      <ReadList linksVisible={linksVisible} />
      {/* {props.lang === "en" && (
        <ReadBlogBtnWrapper>
          <AnimatedReadButton pose={blogBtnVisible ? "visible" : "hidden"}>
            <CentredButton
              message={WEBSITE_TEXT_ABOUT.read.blogButton}
              path={"/blog"}
            />
          </AnimatedReadButton>
          <Waypoint onEnter={showBlogButton} />
        </ReadBlogBtnWrapper>
      )} */}
      <AnimatedContent pose={!props.reload ? "visible" : "hidden"}>
        <SectionLinks
